...
const all_options =
  JSON.parse(Buffer.from(process.env.CONFIG || "", "base64").toString()) || [];
...
async function main() {
  if (process.env.SKIP_DOWNLOAD !== "1") {
    ...
    for (let i = 0; i < all_options.length; i++) {
      ...
      console.log(
        "[START] scraping for site '" + element.options.companyId + "'"
      );
      try {
        ...
        if (scrapeResult.success) {
          ...
          await webhookErrorWait(
            `[ERROR-SCRAPE-LOGIC] - ${element.options?.companyId} - ${scrapeResult.errorType}`
          );
        ...
        await webhookErrorWait(
          `[ERROR-SCRAPE-FATAL] - ${
            element.options?.companyId
          } - scraping failed for the following reason: ${
            e.message
          }\n Full Error:\n ${JSON.stringify(e)}`
        );
      ...
      for (let i = 0; i < group_data_array.length; i++) {
        ...
        console.log(
          "Saving '" +
            ...
            group_array.length +
            ", KB:" +
            Math.round(dataToSave.length / 1024)
        );
        fs.writeFileSync(savePath, dataToSave);
      }
    } catch (e) {
      await webhookErrorWait(
        `saving failed for the following reason: ${
          e.message
        }\n Full Error:\n ${JSON.stringify(e)}`
      );
    }
  ...
  if (process.env.S3_BUCKET) {
    ...
    try {
      ...
      if (fs.existsSync(saveBasePath)) {
        diffsTXNs = await uploadFolder(
          saveBasePath,
          process.env.S3_BUCKET,
          process.env.S3_FOLDER || "bank-scrape"
        );
      } else {
      ...
      await webhookErrorWait(
        "[ERROR_UPLOAD_S3] " + e + ", " + JSON.stringify(e),
        e
      );
    }
...
main()
  ...
  .catch((e) => webhookError("ERROR_LOCAL " + e + "," + JSON.stringify(e)));
